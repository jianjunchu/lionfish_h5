<template>
  ...
  <div class="wrap">
    <div class="cate-list" v-if="rushList.length" v-for="(item ,index) in rushList" :key="item.id">
      ...
      <div class="list">
        <i-rush-spu @authModal="authModal" @changeCartNum="changeCartNum" @openSku="openSku" @vipModal="vipModal"
                    class="item" :needAuth="needAuth" :reduction="reduction" :spuItem="goodsItem" @stopClick="stopClick"
                    width250="true" v-if="item.list.length" v-for="(goodsItem ,idx) in item.list"
                    :key="idx"></i-rush-spu>
      </div>
      <i-load-more iClass="loadMore" :loading="loadMore" :tip="loadText"
                   v-if="!loadOver&&!showEmpty&&loadMore"></i-load-more>
    </div>
    ...
    <i-tabbar ref="tabbar" :cartNum="cartNum" class="tabbar {{isIpx?'pb20':''" currentIdx="-1"
              v-if="is_show_cate_tabbar==1"></i-tabbar>
  </div>
    ...
    export default {
        ...
        methods: {
                    ...
                    wx.showLoading(), app.util.request({
                        ...
                        success: function (t) {
                            if (wx.hideLoading(), 0 == t.code) {
                                var a = t, e = a.full_money, i = a.full_reducemoney, n = a.is_open_fullreduction,
                                    o = a.list, s = a.is_show_cate_tabbar, u = {
                                        full_money: e,
                                        full_reducemoney: i,
            ...
            onShareAppMessage: function (t) {
                ...
                return {
                    title: e,
                    path: "ulink_comshop/pages/type/details?id=" + this.$data.id + "&share_id=" + i,
                    success: function (t) {
                    },
                    fail: function (t) {
                    }
