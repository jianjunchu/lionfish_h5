<template>
  <div class="wrap">
    <div class="score bg-f p30 mb10" :style="info.modify_signinreward_logo?'background:url('+info.modify_signinreward_logo+') no-repeat top center;background-size:100%;':''">
      <div class="i-flex i-flex-spb">
        <div></div>
        <div data-link="/ulink_comshop/pages/user/rule?type=signin" @click="goLink" class="navItem bule">{{$t('score.huodongguize')}}</div>
      </div>
      <div class="i-flex-col i-flex-alc">
        <div class="tit">{{$t('score.dangqianjifen')}}</div>
        <div class="num i-flex i-flex-alc" data-link="/ulink_comshop/moduleA/score/scoreDetails" @click="goLink">
          {{info.score || 0}}
          <div class="fsz-28" style="margin-top:2vw;margin-left:1vw;">{{$t('score.mingxi')}}<span class="iconfont icon-youjiantou fsz-26"></span></div>
        </div>
      </div>
    </div>

    <div class="sign bg-f p30 mb10 i-flex-col i-flex-alc" v-if="info.isopen_signinreward==1">
      <div class="signBtn disabled" v-if="info.today_is_signin==1">{{$t('score.yiqiandao')}}</div>
      <div class="signBtn" v-else @click="signIn">{{$t('score.qiandao')}}</div>
      <div class="tip">{{$t('score.yilianxuqiandao')}}
        <text class="bule">{{info.has_continuity_day || 0}}</text>{{$t('score.tian')}}</div>
      <div class="list i-flex i-flex-spb" v-if="info.show_day_arr">
        <div :class="'list-item' + item.is_signin==1?'on':''" v-for="(item,index) in info.show_day_arr" :key="item.id">
          <text class="iconfont icon-wanchenggouxuan2" v-if="item.is_signin==1"></text>
          <text class="num" v-else>+{{dayScore[index]}}</text>
          <div class="datetime">{{item.is_today==1?$t('score.jinri'):item.date}}</div>
        </div>
      </div>
    </div>

    <div class="reward p30 bg-f mb20" v-if="dayScore.length&&false">
      <div class="tit weight">{{$t('score.lianxuqiandaojiangli')}}：</div>
      <div class="viplist">
        <div class="list" v-if="dayScore.length>0">
          <div class="list-item i-flex-col i-flex-spb" v-for="(item,index) in dayScore" :key="item.id">
            <div style="font-size:2.6vw;">{{$t('score.连续')}}{{index+1}}{{$t('score.tian')}}</div>
            <div class="bule weight" style="font-size:5.4vw;">
              {{item}}
              <text style="font-size:5.4vw;">{{$t('score.jifen')}}</text>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- 猜你喜欢 -->
    <div class='like i-class' v-if="list.length&&noData!=1">
      <div class='like-title'>——{{$t('score.jifenduihuan')}}——</div>
      <div class='like-list'>
        <guess-like @openSku="openSku" :item="item" v-for="(item,index) in list" :key="item.id" @vipModal="vipModal"></guess-like>
      </div>
    ...
    <i-modal :scrollUp="false" :visible="showSignModal">
      <div class="signTip">
        ...
        <div class="signTipCon text-center">
          <div class='state'>{{$t('score.qiandaochenggong')}}</div>
          <div class='integral'>{{$t('score.huode')}}{{reward_socre}}{{$t('score.jifen')}}</div>
          <div class='signTipBnt' @click='handleTipModal'>{{$t('score.haode')}}</div>
        </div>
      </div>
    </i-modal>
    ...
    <sku :visible="visible" :skuList="skuList" :cur_sku_arr="cur_sku_arr" @cancel="closeSku" :sku_val="sku_val" :sku="sku" buyType="integral" :goodsid="addCar_goodsid" @vipModal="vipModal"></sku>
...
<script>
  ...
  import {scoreCartMixin} from '../../mixin/scoreCartMixin.js';
  import GuessLike from '../components/score-guess-like/index.vue'
  var app,wx
  export default {
    mixins: [scoreCartMixin,GlobalMixin],
    name:'supply-recruit',
    components: {'guess-like': GuessLike},
    data() {
      return {
        needAuth:false,
        showAuthModal:false,
    ...
    created: function() {
      ...
      wx.setNavigationBarTitle({
        title: that.$t('common.jifen'),
        showBack: true
      })
      that.loadText = that.$t('score.jiazaizhong');
      this.onLoad()
      this.onShow()
    },
    methods: {
      ...
      signIn: function () {
        ...
        app.util.request({
          ...
          success: function (res) {
            wx.hideLoading();
            if (res.code == 0) {
              let { score, continuity_day, reward_socre } = res;
              let info = that.info;
              info.score = score;
              ...
              that.info = info;
              that.showSignModal = true;
              that.reward_socre = reward_socre;
              // app.util.message('签到成功', '', 'success');
            } else{
              wx.showModal({
                title: that.$t('score.jiazaishibai'),
                content: e.msg,
                showCancel: false
              })
              wx.navigateTo({
      ...
      handleTipModal: function(){
        this.showSignModal = !this.data.showSignModal;
      },
        ...
        app.util.request({
          ...
          success: function (res) {
            if (res.code == 0) {
              ...
              that.loadMore = false;
              that.noMore = false;
              that.loadText = that.$t('score.meiyougengduojilule');
            }
          }
        })
      },
...
.signBtn {
  background: linear-gradient(45deg, #72adf0, #145aa9);
  width: 20vw;
  height: 20vw;
  border-radius: 50%;
  color: #fff;
...
.sign .list .list-item::before {
  position: absolute;
  content: "";
  border: 0.4vw solid rgb(59, 140, 232);
  left: -3.6vw;
  width: 5.5vw;
  top: 2.5vw;
}

.sign .list .list-item.on::before {
  content: "";
  border: 0.4vw solid #145aa9;
}

.sign .list .list-item:first-child::before {
  content: none;
}

.sign .list .list-item .num {
  position: relative;
  z-index: 1;
  ...
  margin-bottom: 1vw;
  font-size: 2.4vw;
  background: rgb(59, 140, 232);
  color: #fff;
}

.sign .list .list-item .iconfont {
  position: relative;
  z-index: 1;
  display: inline-block;
  text-align: center;
...
.signTip .signTipCon .state{font-size:34rpx;color:#fff;margin-top:150rpx;}
.signTip .signTipCon .integral{font-size:30rpx;color:rgba(255,255,255,0.6);margin-top:9rpx;}
.signTip .signTipCon .signTipBnt{font-size:30rpx;color:#eb4331;width:260rpx;height:76rpx;background-color:#f8d168;border-radius:38rpx;line-height:76rpx;margin:48rpx auto 0 auto;}
